#!/usr/bin/env bash
# shellcheck disable=SC2034,SC2155,SC2153,SC2209,SC2002,SC2005
# ==================================================================
# install.sh
# ==================================================================
# Install Ruby Feature
#
# File:         install.sh
# Author:       Ragdata
# Date:         26/06/2023
...
if [ "${SKIP_RBENV_RBUILD}" != "true" ]; then
    ...
    if [ "${USERNAME}" != "root" ]; then
        ...
        find "/home/${USERNAME}/.rbenv" -type d -print0 | xargs -0 -n 1 chmod g+s
    fi
fi

chown -R "${USERNAME}:rvm" "/usr/local/rvm/"
chmod -R g+r+w "/usr/local/rvm/"
find "/usr/local/rvm/" -type d -print0 | xargs -0 -n 1 chmod g+s

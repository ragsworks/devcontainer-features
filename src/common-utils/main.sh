...
# VARIABLES
# ==================================================================
INSTALL_ZSH="${INSTALLZSH:-"false"}"
CONFIGURE_ZSH_AS_DEFAULT="${CONFIGUREZSHASDEFAULT:-"false"}"
INSTALL_OH_MY_ZSH="${INSTALLOHMYZSH:-"false"}"
...
# ==================================================================
install_debian() {
	# put the app into non-interactive mode
	export DEBIAN_FRONTEND=noninteractive

	local packages libssl_package package_list

	packages="apt-utils openssh-client gnupg2 dirmgr iproute2 procps lsof htop net-tools psmisc curl tree wget rsync ca-certificates unzip bzip2 zip nanp vim-tiny less jq lsb-release apt-transport-https \
					dialog libc6 libgcc1 libkrb5-3 libgssapi-krb5-2 libicu[0-9][0-9] liblttng-ust[0-9] libstdc++6 zlib1g locales sudo ncdu man-db strace manpages manpages-dev init-system-helpers"
	...
	# Ensure at least the en_US.UTF-8 UTF-8 locale is available = common need for both applications
...
install_alpine() {
	apk update
	apk add --no-cache openssh-client gnupg procps lsof htop net-tools psmisc curl wget rsync ca-certificates unzip zip nano vim less jq libgcc libstdc++ krb5-libs libintl libssl1.1 lttng-ust tzdata userspace-rcu \
		zlib sudo coreutils sed grep which ncdu shadow strace
...
if [ "${RC_SNIPPET_ALREADY_ADDED}" != "true" ]; then
    ...
    cat "${FEATURE_DIR}/scripts/devcontainers.bash-theme" >> "${user_home}/.bashrc"
    if [ "${USERNAME}" != "root" ]; then
        cat "${FEATURE_DIR}/scripts/devcontainers.bash-theme" >> "/root/.bashrc"
...
# code shim, it falls back to code-insiders if code is not available
